...
def create_class_indexing():
    dict = {}
    for i in range(0, 256):
        dict.update({str(i + 1): i})
    return dict
...
def create_dataset(dataset_path, do_train, config, device_target, batch_size=32, run_distribute=False):
    ...
    Returns:
        dataset
    """
    # 创建caltech256数据集的标签索引
    dict = create_class_indexing()
    if device_target == "GPU":
        if do_train:
            if run_distribute:
                ...
                data_set = ds.ImageFolderDataset(dataset_path, num_parallel_workers=8,
                                                 class_indexing=dict,shuffle=True)
        else:
            data_set = ds.ImageFolderDataset(dataset_path, num_parallel_workers=8,
                                             class_indexing=dict, shuffle=True)
    else:
